...
function LuopanDial({
  ...
  const needle = polarToCartesian(normalizeHeading(headingDeg), 415);
  const counterNeedle = polarToCartesian(normalizeHeading(headingDeg + 180), 276);
    ...
    <div className="relative mx-auto flex aspect-square w-full max-w-[760px] items-center justify-center overflow-hidden rounded-full">
      ...
      <svg viewBox="0 0 1000 1000" className="absolute inset-0 h-full w-full">
        <defs>
          ...
          <marker id="needle-tip" markerWidth="12" markerHeight="12" refX="10" refY="6" orient="auto">
            <path d="M0,0 L12,6 L0,12 Z" fill="currentColor" />
          </marker>
          <marker id="needle-tail" markerWidth="10" markerHeight="10" refX="1" refY="5" orient="auto">
        ...
        <line x1="500" y1="500" x2={needle.x} y2={needle.y} stroke="rgb(212,174,96)" strokeWidth="9" strokeLinecap="round" markerEnd="url(#needle-tip)" filter="url(#luopan-soft-shadow)" />
        <line x1="500" y1="500" x2={counterNeedle.x} y2={counterNeedle.y} stroke="rgba(190,18,60,0.72)" strokeWidth="5" strokeLinecap="round" markerEnd="url(#needle-tail)" />
  ...
  return (
    <div className={getSectorStyle(active, sitting, false, [1, 6, 8].includes(cell.periodStar) || [1, 6, 8].includes(cell.mountainStar) || [1, 6, 8].includes(cell.waterStar))}>
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-text-secondary-light dark:text-text-secondary-dark">
            {label}
          </p>
          <p className="mt-1 text-sm font-bold text-text-primary-light dark:text-text-primary-dark">{cell.positionVi}</p>
        </div>
        <span className="rounded-full border border-border-light/60 px-2 py-1 text-[11px] font-bold text-text-secondary-light dark:border-white/10 dark:text-text-secondary-dark">
          {cell.position}
        </span>
      </div>

      <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px]">
        <div className="rounded-lg bg-black/5 px-2 py-1 dark:bg-white/5">
          <span className="block text-[10px] uppercase tracking-wide text-text-secondary-light dark:text-text-secondary-dark">Vận</span>
          <span className="block text-sm font-bold">{cell.periodStar}</span>
        </div>
        <div className="rounded-lg bg-black/5 px-2 py-1 dark:bg-white/5">
          <span className="block text-[10px] uppercase tracking-wide text-text-secondary-light dark:text-text-secondary-dark">Sơn</span>
          <span className="block text-sm font-bold">{cell.mountainStar}</span>
        </div>
        <div className="rounded-lg bg-black/5 px-2 py-1 dark:bg-white/5">
          ...
          <span className="block text-sm font-bold">{cell.waterStar}</span>
        </div>
      </div>

      <div className="mt-3 flex flex-wrap gap-1.5 text-[10px] font-semibold">
        <span className="rounded-full bg-slate-900/5 px-2 py-1 dark:bg-white/5">Niên {cell.annualStar ?? '—'}</span>
        <span className="rounded-full bg-slate-900/5 px-2 py-1 dark:bg-white/5">Nguyệt {cell.monthlyStar ?? '—'}</span>
      </div>
    </div>
  );
}
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <section className="surface-panel p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-right text-xs font-semibold text-text-secondary-light dark:text-text-secondary-dark">
